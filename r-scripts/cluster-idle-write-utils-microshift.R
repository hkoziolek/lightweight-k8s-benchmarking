...
timestamps <- exp_collection$find(query="{\"experimentID\": 161}", fields='{"_id":false, "experimentID":true, "timestamp":true}')
...
get.utils <- function(name, t1, t2){
  cpu_util_master <- get.netdata("kerberos", "system", "cpu", "system.cpu", "idle", timestamp_start, timestamp_end)
  ...
  mem_util_master <- get.netdata("kerberos", "system", "ram", "system.ram", "used", timestamp_start, timestamp_end)
  ...
  disk_util_master <- get.netdata("kerberos", "disk_util", "sda", "disk.util", "utilization", timestamp_start, timestamp_end)
  disk_util_master <- subset (disk_util_master, select = -timestamp)
  disk_util_master['metric']=paste("'Disk Controller",name,"'")
 
  util <- rbind(cpu_util_master, mem_util_master, disk_util_master)
  return(util)
  
}

util <- get.utils("MicroShift", timestamp_start, timestamp_end)
